...
from core.forms import EquipmentPhotoForm, SiteForm
from core.models import Equipment, Log


@login_required
def sites(request):
    context = {"breadcrumb":{"parent":"Site","child":"Sites"}}
    return render(request, 'core/site/sites/sites.html', context=context)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                log.save()                        
            success(request, 'Site was created successfully!')
            return redirect('sites')
    else:
        form = SiteForm()

    context = {
        "breadcrumb":{"parent":"Site","child":"Create Site"},
        "form": form
    }
    return render(request, 'core/site/create_new/create.html', context=context)
